    ...
    *,
    ...
    def _step(y):
        with mx.stream(generation_stream):
            ...
            return pred.squeeze(0), logits

    T = y.shape[-1]
    for n in range(0, T):
        pred, logits = _step(y[n:n+1])
        mx.async_eval(pred, logits)

        yield pred.item(), logits
...
def stream_classify(
    ...
    **kwargs,
) -> Generator[ClassificationResponse, None, None]:
    """
    ...
    class_generator =  classifier_step(prompt, model, **kwargs)

    with wired_limit(model, [generation_stream]):
        tic = time.perf_counter()
        for n, (pred, logits) in enumerate(class_generator):

            yield ClassificationResponse(
                label = pred,
                probs = logits,
                generation_tokens=n,
...
def classify(
    ...
    **kwargs,
    ...
    for response in stream_classify(model, tokenizer, prompt, **kwargs):
        # if verbose:
            # print(response.label, response.probs, end="", flush=True)
        # print(response.label, response.probs)
        preds.append(response.label)
        probs.append(response.probs)

    if verbose:
        print()
        print("=" * 10)
        print(
            f"Classification: {response.generation_tokens} tokens, "
...
def inject_init_prompt(
    ...
    *,
    ...
    print("Inject time: ", time.perf_counter() - t0)
